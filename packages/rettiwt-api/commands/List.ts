...
function createListCommand(rettiwt: Rettiwt): Command {
  ...
  list.command('members')
    ...
    .action(async (id: string, count?: string, cursor?: string) => {
      try {
        const members = await rettiwt.list.members(id, count ? Number.parseInt(count) : undefined, cursor)
        output(members)
      }
      catch (error) {
        output(error)
  ...
  list.command('tweets')
    ...
    .action(async (id: string, count?: string, cursor?: string) => {
      try {
        const tweets = await rettiwt.list.tweets(id, count ? Number.parseInt(count) : undefined, cursor)
        output(tweets)
      }
      catch (error) {
        output(error)
