...
function createListCommand(rettiwt: Rettiwt): Command {
  ...
  const list = createCommand('list').description(
    'Access resources related to lists',
  )
  ...
  list
    ...
    .action(async (id: string, count?: string, cursor?: string) => {
      try {
        const members = await rettiwt.list.members(
          id,
  ...
  list
    ...
    .action(async (id: string, count?: string, cursor?: string) => {
      try {
        const tweets = await rettiwt.list.tweets(
          id,
          count ? Number.parseInt(count) : undefined,
          cursor,
        )
