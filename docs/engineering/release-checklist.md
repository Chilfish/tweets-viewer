...
> 执行环境：本地构建 + 生产环境冒烟 / Vercel 部署后验证。

## 版本纪律

- 版本号单源在根 `package.json` 的 `version` 字段；发布时同步 `CHANGELOG.md`，不要在多处手写副本
- 发版走 `docs/engineering/git-workflow.md`：CHANGELOG 更新 → 版本号 → PR → tag → GitHub Release
...
> `NODE_ENV` 由 `cross-env` 在各包脚本内固定（`test`/`dev`/`build`），不依赖宿主环境。
> 若本地 `echo $env:NODE_ENV` 为 `production`，测试与 dev 仍应正常（见 postmortem 005）。
...
- [ ] `bun run build:client` 构建通过（无报错）
- [ ] 首页 / `/tweets/:name` / `/media/:name` / `/search` / `/memo` / `/ins` 全部路由可达
- [ ] 首屏正常渲染（无 Hydration 错误）；动态内容客户端渲染（SPA-first，见 ADR-010）
- [ ] 无限滚动（keyset cursor 续载）+ 分页器交互正常；URL `page` 随续载同步
- [ ] 日期范围筛选（DateRangeFilter）与按年浏览（YearNavigator）在浅色/深色模式正常
- [ ] 分享按钮：普通点击走 Web Share / 降级新标签；右键与修饰键保留原生链接行为
- [ ] 移动端（<768px）布局无溢出；底栏保持 5 tab；触控目标 ≥44px
- [ ] 控制台无 500/404 API 错误

## API（Cloudflare Workers, `tweet-api.chilfish.top`）

- [ ] `bun run deploy` 部署成功，`wrangler.json` 配置（DATABASE_URL, TWEET_KEYS, RATE_LIMITER）
- [ ] 核心端点冒烟：`GET /v3/tweets/get/:name`、`/v3/tweets/medias/:name`、`/v3/tweets/search?q=`、`/v3/tweets/last-years-today`、`/v3/users/all`、`/v3/ins/:name`
- [ ] 日期范围 `start`/`end`（含 date-only 值）返回正确
- [ ] 分页边界：末页 `meta.hasMore=false` 正确；keyset 游标续载无重复/漏项
