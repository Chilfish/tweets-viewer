...
移动端遵循 Apple 范式：**底栏 = 全局导航，顶栏 = 上下文标题 + 账户操作**。桌面端使用侧边栏承载全局导航与用户选择。

| 端 | 组件 | 职责 | 呈现 |
|---|---|---|---|
| 桌面 | `sidebar.tsx` | 全局导航 + 用户选择器（完整版） | 左侧固定栏 |
| 移动 | `top-nav.tsx` | 上下文标题 + 账户操作 + 主题切换 | 顶部 Sticky Glass |
| 移动 | `bottom-nav.tsx` | 全局导航（主页/媒体/IG/那年今日/搜索） | 底部 Tab Bar（图标 + 文字标签） |

- **顶栏 (TopBar)**：
  - 上下文标题：用户页显示 `@curUserName`，首页显示站点名。
  - 账户操作：用户切换以**精简头像按钮**呈现（无用户名文字），点击展开用户列表；**禁止**顶栏内 title 与用户选择器同时以文字形式重复显示用户名。
- **底栏 (Tab Bar)**：
  - 始终悬浮底部（仅移动端，`md:hidden`），以「图标 + 文字标签」呈现（iOS Tab Bar 范式）。
  - 不含当前页面上下文信息（上下文只属于顶栏）。
- **用户切换**：移动端入口 = 顶栏头像按钮；桌面端入口 = 侧边栏底部 `UserSelector`（完整版）。
