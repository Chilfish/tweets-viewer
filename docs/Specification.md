...
- **全量模式（`name` 缺省）**: 访问 `/memo`（无用户）时，展示**所有归档用户**的同月同日回忆，采用
  「按年分组 → 组内按用户分小节」的两层结构：年为主分组（章，延续时间叙事与日期主角），用户为小节
  （节，复用全局搜索的作者分隔线：头像 + 显示名 + `@handle`）。同年内不同用户的小节顺序随时间线自然排列；
  用户头像/显示名优先取自已归档用户列表（`useUserStore.users`），推文内嵌 user 字段作兜底。
