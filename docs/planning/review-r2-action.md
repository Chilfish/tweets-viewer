...
| # | 任务 | 设计决策 | 状态 |
|---|---|---|---|
| P1-1 | 首页重做：去硬编码 `240y_k`，加用户/最近浏览入口，讲清"完整人生归档" | ① 删除硬编码 CTA；② hero/meta 文案定位改为「完整人生归档」（时间/记忆/完整性），去掉"第三方查看器"自我矮化；③ `useUserStore` 新增持久化 `recentUsers`（去重、上限 6）+ `pushRecentUser`，layout loader 解析 activeUser 时记录；④ 首页新增「继续浏览 / 最近浏览 / 归档用户」入口 | 🟡 处置中 |
| P1-2 | "那年今日"升级为仪式感体验（日期大字 + 按年分组 + 首页入口） | ① 仪式感头部：大字日期「8月9日」+ 回忆总数（取 `meta.total`）；② 推文按年分组（`groupTweetsByYear` 纯函数，连续段分组 + 年分隔线，不破坏时间线顺序）；③ 首页「那年今日」卡片入口（目标用户 = activeUser ?? recentUsers[0] ?? users[0]） | 🟡 处置中 |
| P1-3 | 移动端导航职责重整（底栏全局 / 顶栏仅上下文） | 待处置 | ⬜ |
