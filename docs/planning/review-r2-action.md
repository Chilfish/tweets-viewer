...
| P1-3 | 移动端导航职责重整（底栏全局 / 顶栏仅上下文） | ① 顶栏 = 上下文标题（`@curUserName`）+ 账户操作（用户切换精简为纯头像按钮，去重复用户名文字）+ 主题切换；② 底栏 = 全局导航，恢复「图标 + 文字标签」（iOS Tab Bar 范式）；③ 桌面 Sidebar 不变；④ 规范写入 Specification §5.4 | ✅ 已完成 |
