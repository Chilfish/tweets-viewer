...
1. **先写 commit message 再写代码**（详见 `docs/engineering/git-workflow.md`）。每个 commit 是单一关注点的原子提交，避免"上帝 commit"。当 diff >10 文件或 >200 行时必须拆分。模块创建、功能实现、配置修改、文档更新分开 commit。

2. **文档先行（docs-first）**：每个任务第一步先更新相关文档（开发日志 `docs/development-log/README.md`、任务状态 `docs/planning/action-plan.md`、方案文档），再开始写代码。实施过程中随实际反馈同步修改文档、记录开发日志与踩坑，而非事后补记。已完成阶段的规划文档归档于 `docs/archive/`——**历史记录，不主动读取**；活跃文档只在 `docs/planning/` 与 `docs/` 根。

3. **开写代码前先读尸检报告**（`docs/postmortem/README.md`）：历史踩坑沉淀于此，写码/重构前对照「高频雷区」自查。遇到新的返工/事故按 `docs/postmortem/TEMPLATE.md` 沉淀一条 postmortem。

4. **规格驱动（SDD）**：功能行为变更必须先更新 `docs/Specification.md`（事实来源），再动代码；API 变更同步更新 `docs/API_DOCUMENTATION.md`。

5. **测试先行**：新增查询/工具模块先写 Vitest 测试（包内 `__tests__/` 或 `.test.ts`），跑 `bun --cwd <pkg> test` 确认绿再实现。
...
- **pre-commit**：ESLint（`--fix` + stage_fixed）
- **pre-push**：`bun run build:client`
...
- 规划（`docs/planning/`）— 架构决策 (ADR) / 行动计划
- 工程规范（`docs/engineering/`）— 代码规范 / Git 工作流 / 部署清单
- 开发日志（`docs/development-log/`）— 按天记录
- 尸检报告（`docs/postmortem/`）— 踩坑沉淀
